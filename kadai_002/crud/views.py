...
class CreateReservationView(LoginRequiredMixin, View):
    def post(self, request, restaurant_id):
        ...
        try:
            reservation_datetime = parse_datetime(reservation_datetime_str)
            if not reservation_datetime:
                raise ValueError("Invalid date/time format.")
        except ValueError:
            messages.error(request, "予約の日時が無効です。正しい日時を選択してください。")
            return redirect('restaurant_detail', pk=restaurant.id)
        
        # 定休日のチェック
        print(f"定休日チェック: {restaurant.is_closed_on(reservation_datetime.date())}, 日付: {reservation_datetime.date()}")
        if restaurant.is_closed_on(reservation_datetime.date()):
            messages.error(request, "選択した日は定休日です。別の日付を選択してください。")
            return redirect('restaurant_detail', pk=restaurant.id)

        if not (restaurant.start_time <= reservation_time <= restaurant.end_time):
            messages.error(request, "選択した時間は営業時間外です。")
            return redirect('restaurant_detail', pk=restaurant.id)
